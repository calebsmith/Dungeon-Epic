...
void map_display(Map& level, Viewpoint& camera, Surface& images){
    ...
    for(y = 0; y < h; y++)
    {
        ...
        for(x = 0; x < w; x++)
        {
            ...
            thistile.x = 0;            
            blit(images.tilesheet, thistile, images.buffer,tilexy);      
        }        
    }
    return;
...
//    Uint32 colorkey = SDL_MapRGB(images.playersheet->format, 0x000000,0x000000,0x000000);
//    SDL_SetColorKey(images.playersheet, SDL_SRCCOLORKEY,colorkey);
    
     
//    SDL_SetColorKey(images.playersheet, SDL_SRCCOLORKEY, SDL_MapRGB(images.buffer->format, 0, 0, 0));
//    
   //SDL_SetAlpha(images.playersheet, SDL_SRCALPHA, 0);
